...
fn get_network_from_nmcli_line(line: &str) -> Result<WirelessNetwork, IndividualParseError> {
    ...
    let enc_txt = tokens.next().ok_or(IndividualParseError::MissingNmcliSeparator)?;
    let signal_strength_txt = tokens.next().ok_or(IndividualParseError::MissingNmcliSeparator)?;
    let essid_txt = tokens.next().ok_or(IndividualParseError::MissingNmcliSeparator)?;
