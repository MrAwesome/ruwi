    ...
    O: Send + Sync + Global + LinuxNetworkingInterface + AutoSelect + WifiDataGatherer,
{
    ...
    loop {
        ...
        let selected_network = sorted_networks.select_network(options);
        if manual_refresh_requested(&selected_network) {
...
#[inline]
pub(super) fn await_thread<T>(handle: thread::JoinHandle<T>) -> Result<T, RuwiError> {
